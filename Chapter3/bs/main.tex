...
Several protocols have been developed to profile average DNA methylation levels in a bulk collection of cells (\Cref{sec:intro_proto}). However, bulk profiling protocols are unable to estimate methylation heterogeneity between single cells, which is critical for studying embryonic development, cancer progression, and pluripotent stem cells. In the following, we will describe scBS-seq, an accurate and reproducible method for profiling DNA methylation in single cells, and a statistical model for estimating methylation heterogeneity in cell populations across the entire genome.
